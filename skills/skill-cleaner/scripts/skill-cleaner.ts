...
const help = `Usage: skill-cleaner.ts [options]
...
function skillRootScope(root: string): string {
  ...
  if (normalized.includes("/.config/opencode/skill")) return "opencode";
  if (normalized.includes("/.openclaw/skills")) return "openclaw";
  if (normalized.includes("/.agents/skills")) return "universal";
  if (normalized.startsWith(process.cwd().split(path.sep).join("/"))) return "project";
  return "extra";
}

function deletePriority(skill: Skill): number {
  if (skill.path.includes("/.codex/skills/.system/")) return 0;
  if (["codex", "pi", "claude-code", "opencode", "openclaw"].includes(skill.scope)) return 1;
  if (skill.scope === "universal") return 2;
  if (skill.scope === "codex-plugin") return 3;
  if (skill.scope === "project") return 4;
  return 5;
}
...
function pluginPrefixFor(file: string): string | null {
  ...
  if (cache >= 0 && skills > cache + 1) {
    ...
    if (maybePlugin && !maybePlugin.startsWith("plugin-install-")) return maybePlugin;
...
function projectSkillRoots(start = process.cwd()): string[] {
  const roots: string[] = [];
  ...
  while (true) {
    ...
    if (exists(path.join(current, ".git")) || parent === current) break;
    current = parent;
  }
  return roots;
}
...
function skillOrderRank(skill: Skill): number {
  ...
  if (["codex", "pi", "claude-code", "opencode", "openclaw"].includes(skill.scope)) return 2;
  if (skill.scope === "universal") return 3;
  if (skill.scope === "project") return 4;
  return 5;
}
...
function duplicateDeleteSuggestions(groups: [string, Skill[]][]): string[] {
  ...
  for (const [name, list] of groups.slice(0, 10)) {
    ...
    if (keep.scope === "codex-plugin") continue;
